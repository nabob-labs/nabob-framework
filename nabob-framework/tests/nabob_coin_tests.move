...
module nabob_framework::nabob_coin_tests {
    ...
    public fun mint_apt_fa_to_for_test<T: key>(store: Object<T>, amount: u64) {
        fungible_asset::deposit(store, nabob_coin::mint_apt_fa_for_test(amount));
    }
    ...
    public fun mint_apt_fa_to_primary_fungible_store_for_test(
        ...
        primary_fungible_store::deposit(owner, nabob_coin::mint_apt_fa_for_test(amount));
    }

    #[test(nabob_framework = @nabob_framework)]
    fun test_apt_setup_and_mint(nabob_framework: &signer) {
    ...
    fun test_fa_helpers_for_test() {
        ...
        assert!(object::object_exists<Metadata>(@nabob_fungible_asset), 0);
        mint_apt_fa_to_primary_fungible_store_for_test(@nabob_framework, 100);
