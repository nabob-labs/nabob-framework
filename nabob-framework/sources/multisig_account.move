...
module nabob_framework::multisig_account {
    use nabob_framework::account::{Self, SignerCapability, new_event_handle, create_resource_address};
    ...
    use nabob_framework::timestamp::now_seconds;
    ...
    /// account.
    const DOMAIN_SEPARATOR: vector<u8> = b"nabob_framework::multisig_account";
    ...
    fun create_multisig_account(owner: &signer): (signer, SignerCapability) {
        ...
        // Register the account to receive BOB as this is not done by default as part of the resource account creation
        // flow.
        if (!coin::is_account_registered<NabobCoin>(address_of(&multisig_signer))) {
            coin::register<NabobCoin>(&multisig_signer);
        };
    ...
    #[test_only]
    use nabob_framework::coin::{destroy_mint_cap, destroy_burn_cap};
    ...
    fun setup() {
        ...
        let (burn, mint) = nabob_coin::initialize_for_test(framework_signer);
