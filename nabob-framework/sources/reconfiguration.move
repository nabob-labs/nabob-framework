...
module nabob_framework::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;
    ...
    friend nabob_framework::consensus_config;
    friend nabob_framework::execution_config;
    friend nabob_framework::gas_schedule;
    friend nabob_framework::genesis;
    friend nabob_framework::version;
    ...
    /// Publishes `Configuration` resource. Can only be invoked by nabob framework account, and only a single time in Genesis.
    public(friend) fun initialize(nabob_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(nabob_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            nabob_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(nabob_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(nabob_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(nabob_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_nabob_framework(nabob_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(nabob_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@nabob_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@nabob_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@nabob_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@nabob_framework).epoch
    }
