...
module nabob_framework::storage_gas {
    ...
    public(friend) fun set_config(nabob_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_nabob_framework(nabob_framework);
        *borrow_global_mut<StorageGasConfig>(@nabob_framework) = config;
    }
    ...
    public fun initialize(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        assert!(
            !exists<StorageGasConfig>(@nabob_framework),
        ...
        move_to(nabob_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@nabob_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(nabob_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@nabob_framework);
        let gas = borrow_global_mut<StorageGas>(@nabob_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@nabob_framework);
