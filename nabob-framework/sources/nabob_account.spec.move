spec nabob_framework::nabob_account {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the creation of an Nabob account the following rules should hold: (1) the authentication key
    /// should be 32 bytes in length, (2) an Nabob account should not already exist for that authentication key, and (3)
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Nabob account, the account should become registered to receive NabobCoin.
    /// Criticality: Critical
    ...
    /// exist; then, register NabobCoin for that account if a particular is unregistered before transferring the amount.
    /// Criticality: Critical
    /// Implementation: The transfer function checks if the recipient account exists. If the account does not exist,
    /// the function creates one and registers the account to NabobCoin if not registered.
    ...
    ///
    /// No.: 7
    /// Requirement: When performing a batch transfer of Nabob Coin and/or a batch transfer of a custom coin type, it
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @nabob_framework || auth_key == @nabob_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        include GuidAbortsIf<NabobCoin>;
        include WithdrawAbortsIf<NabobCoin>{from: source};
        include TransferEnsures<NabobCoin>;

        aborts_if exists<coin::CoinStore<NabobCoin>>(to) && global<coin::CoinStore<NabobCoin>>(to).frozen;
        /// [high-level-req-5]
        ensures exists<nabob_framework::account::Account>(to);
        ensures exists<coin::CoinStore<NabobCoin>>(to);
    }
    ...
    spec assert_account_is_registered_for_bob(addr: address) {
        pragma aborts_if_is_partial;
        aborts_if !account::exists_at(addr);
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<NabobCoin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
                !account::exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @nabob_framework || recipients[i] == @nabob_token);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<NabobCoin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<NabobCoin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<NabobCoin>>(recipients[i]) && global<coin::CoinStore<NabobCoin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<NabobCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::exists_at(recipients[i]) && !exists<coin::CoinStore<NabobCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec register_bob(account_signer: &signer) {
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::exists_at(to) && (to == @vm_reserved || to == @nabob_framework || to == @nabob_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<nabob_framework::account::Account>(to);
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<NabobCoin>();
    }
