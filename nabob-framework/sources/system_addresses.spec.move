spec nabob_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Nabob Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_nabob_framework function ensures that the provided signer belongs to the
    /// @nabob_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotNabobFramework).
    ///
    /// No.: 3
    ...
    spec assert_nabob_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotNabobFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @nabob_framework with error::PERMISSION_DENIED;
    }
