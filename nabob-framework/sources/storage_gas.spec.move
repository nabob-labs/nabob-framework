spec nabob_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@nabob_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@nabob_framework);
    }
    ...
    spec set_config(nabob_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(nabob_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@nabob_framework);
        aborts_if exists<StorageGas>(@nabob_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@nabob_framework);
        ensures exists<StorageGas>(@nabob_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@nabob_framework);
        aborts_if !exists<StorageGas>(@nabob_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@nabob_framework);
    }
