spec nabob_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<NabobCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<NabobCoin> { account: staker };
        let balance = global<coin::CoinStore<NabobCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<NabobCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@nabob_framework);
        requires exists<stake::ValidatorSet>(@nabob_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @nabob_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<nabob_framework::timestamp::CurrentTimeMicroseconds>(@nabob_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@nabob_framework);
        aborts_if exists<stake::AllowedValidators>(@nabob_framework) && !contains(allowed.accounts, resource_addr);
