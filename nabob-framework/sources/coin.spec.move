spec nabob_framework::coin {
    ...
    /// Can only be updated by `@nabob_framework`.
    spec allow_supply_upgrades(_nabob_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@nabob_framework)) {
            let map = global<CoinConversionMap>(@nabob_framework).coin_to_fungible_asset_map;
    ...
    spec fun spec_is_account_registered<CoinType>(account_addr: address): bool {
        ...
        exists<CoinStore<CoinType>>(account_addr) || features::spec_new_accounts_default_to_fa_bob_store_enabled(
        ) || (option::spec_is_some(
            paired_metadata_opt
    ...
    /// The creator of `CoinType` must be `@nabob_framework`.
    ...
    // `account` must be `@nabob_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@nabob_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@nabob_framework);
