...
module nabob_framework::randomness {
    use std::hash;
    use std::option;
    ...
    const DST: vector<u8> = b"NABOB_RANDOMNESS";
    ...
    public fun initialize(framework: &signer) {
        ...
        if (!exists<PerBlockRandomness>(@nabob_framework)) {
    ...
    public(friend) fun on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) acquires PerBlockRandomness {
        ...
        if (exists<PerBlockRandomness>(@nabob_framework)) {
            let randomness = borrow_global_mut<PerBlockRandomness>(@nabob_framework);
    ...
    fun next_32_bytes(): vector<u8> acquires PerBlockRandomness {
        ...
        let input = DST;
        let randomness = borrow_global<PerBlockRandomness>(@nabob_framework);
