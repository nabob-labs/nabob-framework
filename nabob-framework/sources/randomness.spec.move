spec nabob_framework::randomness {

    spec module {
        use nabob_framework::chain_status;
        pragma verify = true;
        invariant [suspendable] chain_status::is_operating() ==> exists<PerBlockRandomness>(@nabob_framework);
    ...
    spec initialize(framework: &signer) {
        ...
        aborts_if framework_addr != @nabob_framework;
    }

    spec on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) {
        use std::signer;
        aborts_if signer::address_of(vm) != @vm;
        ensures exists<PerBlockRandomness>(@nabob_framework) ==> global<PerBlockRandomness>(@nabob_framework).seed == seed_for_new_block;
        ensures exists<PerBlockRandomness>(@nabob_framework) ==> global<PerBlockRandomness>(@nabob_framework).epoch == epoch;
        ensures exists<PerBlockRandomness>(@nabob_framework) ==> global<PerBlockRandomness>(@nabob_framework).round == round;
    }

    spec next_32_bytes(): vector<u8> {
        use std::hash;
        include NextBlobAbortsIf;
        let input = b"NABOB_RANDOMNESS";
        let randomness = global<PerBlockRandomness>(@nabob_framework);
    ...
    spec schema NextBlobAbortsIf {
        ...
        aborts_if !exists<PerBlockRandomness>(@nabob_framework);
    }
    ...
    spec permutation(n: u64): vector<u64> {
        ...
        // aborts_if n > 1 && !exists<PerBlockRandomness>(@nabob_framework);
    }
