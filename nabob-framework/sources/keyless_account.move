...
module nabob_framework::keyless_account {
    ...
    use std::vector;
    use nabob_std::crypto_algebra;
    use nabob_std::ed25519;
    use nabob_framework::chain_status;
    use nabob_framework::system_addresses;

    // The `nabob_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = nabob_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@nabob_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@nabob_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@nabob_framework)) {
                *borrow_global_mut<Configuration>(@nabob_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
