spec nabob_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `nabob_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use std::bcs;
        use nabob_framework::timestamp::{CurrentTimeMicroseconds};
        use nabob_framework::chain_id::{ChainId};
        use nabob_framework::account::{Account};
        use nabob_framework::coin::{CoinStore};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@nabob_framework);
        ...
        aborts_if !exists<CoinStore<NabobCoin>>(gas_payer_addr);
        // property 1: The sender of a transaction should have sufficient coin balance to pay the transaction fee.
        /// [high-level-req-1]
        aborts_if !(global<CoinStore<NabobCoin>>(gas_payer_addr).coin.value >= max_transaction_fee);
    }
    ...
    /// `NabobCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `NabobCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use nabob_framework::coin::{CoinStore, CoinInfo};
        use nabob_framework::optional_aggregator;
        use nabob_framework::transaction_fee::{NabobCoinCapabilities, NabobCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<NabobCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<NabobCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);

        aborts_if !exists<CoinStore<NabobCoin>>(gas_payer);
        ...
        let apt_addr = type_info::type_of<NabobCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<NabobCoin>>(apt_addr).supply;
        ...
        let post post_maybe_apt_supply = global<CoinInfo<NabobCoin>>(apt_addr).supply;
        ...
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<NabobCoin>>(apt_addr);
        ...
        let total_supply = coin::supply<NabobCoin>;
        let post post_total_supply = coin::supply<NabobCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<NabobCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<NabobCoinMintCapability>(@nabob_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<NabobCoin>>(nabob_addr);
        include coin::CoinAddAbortsIf<NabobCoin> { amount: amount_to_mint };
    }
}
