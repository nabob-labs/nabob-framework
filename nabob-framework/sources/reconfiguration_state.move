...
module nabob_framework::reconfiguration_state {
    use std::error;
    use std::string;
    ...
    public(friend) fun is_in_progress(): bool acquires State {
        ...
        let state = borrow_global<State>(@nabob_framework);
    ...
    public(friend) fun on_reconfig_start() acquires State {
        if (exists<State>(@nabob_framework)) {
            let state = borrow_global_mut<State>(@nabob_framework);
            let variant_type_name = *string::bytes(copyable_any::type_name(&state.variant));
