...
module nabob_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(nabob_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by NabobVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@nabob_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@nabob_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@nabob_framework).microseconds
    }
