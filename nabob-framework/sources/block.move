...
module nabob_framework::block {
    use std::error;
    use std::vector;
    use std::option;
    use nabob_std::table_with_length::{Self, TableWithLength};
    use std::option::Option;
    use nabob_framework::randomness;
    ...
    public(friend) fun initialize(nabob_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<CommitHistory>(nabob_framework, CommitHistory {
            max_capacity: 2000,
            next_idx: 0,
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(nabob_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(nabob_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@nabob_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@nabob_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@nabob_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@nabob_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@nabob_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@nabob_framework);
    ...
    public entry fun test_update_epoch_interval(nabob_framework: signer) acquires BlockResource {
        account::create_account_for_test(@nabob_framework);
        initialize(&nabob_framework, 1);
        assert!(borrow_global<BlockResource>(@nabob_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&nabob_framework, 2);
        assert!(borrow_global<BlockResource>(@nabob_framework).epoch_interval == 2, 1);
    }

    #[test(nabob_framework = @nabob_framework, account = @0x123)]
