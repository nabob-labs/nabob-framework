...
module nabob_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use nabob_framework::event::emit;
    use nabob_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@nabob_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@nabob_framework)) {
            borrow_global<DKGState>(@nabob_framework).in_progress
        } else {
            option::none()
        }
    }
