...
module nabob_framework::config_buffer {
    use std::string::String;
    use nabob_std::any;
    use nabob_std::any::Any;
    use nabob_std::simple_map;
    use nabob_std::simple_map::SimpleMap;
    ...
    friend nabob_framework::jwks;
    friend nabob_framework::jwk_consensus_config;
    friend nabob_framework::keyless_account;
    friend nabob_framework::randomness_api_v0_config;
    friend nabob_framework::randomness_config;
    friend nabob_framework::randomness_config_seqnum;
    ...
    public fun initialize(nabob_framework: &signer) {
        system_addresses::assert_nabob_framework(nabob_framework);
        if (!exists<PendingConfigs>(@nabob_framework)) {
            move_to(nabob_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@nabob_framework)) {
            let config = borrow_global<PendingConfigs>(@nabob_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@nabob_framework);
