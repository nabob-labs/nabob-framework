module nabob_framework::genesis {
    use std::error;
    use std::fixed_point32;
    use std::vector;

    use nabob_std::simple_map;
    ...
    use nabob_framework::state_storage;
    use nabob_framework::storage_gas;
    use nabob_framework::timestamp;
    use nabob_framework::transaction_fee;
    use nabob_framework::transaction_validation;
    ...
    fun initialize(
        ...
        // Initialize the nabob framework account. This is the account where system resources and modules will be
        // deployed to. This will be entirely managed by on-chain governance and no entities have the key or privileges
        // to use this account.
        let (nabob_framework_account, nabob_framework_signer_cap) = account::create_framework_reserved_account(@nabob_framework);
        // Initialize account configs on nabob framework account.
        account::initialize(&nabob_framework_account);
        ...
        nabob_governance::store_signer_cap(&nabob_framework_account, @nabob_framework, nabob_framework_signer_cap);
        ...
        while (!vector::is_empty(&framework_reserved_addresses)) {
            ...
            nabob_governance::store_signer_cap(&nabob_framework_account, address, framework_signer_cap);
        };

        consensus_config::initialize(&nabob_framework_account, consensus_config);
        execution_config::set(&nabob_framework_account, execution_config);
        version::initialize(&nabob_framework_account, initial_version);
        stake::initialize(&nabob_framework_account);
        ...
        storage_gas::initialize(&nabob_framework_account);
        gas_schedule::initialize(&nabob_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&nabob_framework_account);

        chain_id::initialize(&nabob_framework_account, chain_id);
        reconfiguration::initialize(&nabob_framework_account);
        block::initialize(&nabob_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&nabob_framework_account);
        timestamp::set_time_has_started(&nabob_framework_account);
    }

    /// Genesis step 2: Initialize Nabob coin.
    fun initialize_nabob_coin(nabob_framework: &signer) {
        ...
        // Give stake module MintCapability<NabobCoin> so it can mint rewards.
        stake::store_nabob_coin_mint_cap(nabob_framework, mint_cap);
        // Give transaction_fee module BurnCapability<NabobCoin> so it can burn gas.
        transaction_fee::store_nabob_coin_burn_cap(nabob_framework, burn_cap);
        // Give transaction_fee module MintCapability<NabobCoin> so it can mint refunds.
        transaction_fee::store_nabob_coin_mint_cap(nabob_framework, mint_cap);
    }

    /// Only called for testnets and e2e tests.
    fun initialize_core_resources_and_nabob_coin(
        ...
        nabob_account::register_apt(&core_resources); // registers APT store
        nabob_coin::configure_accounts_for_test(nabob_framework, &core_resources, mint_cap);
    }

    fun create_accounts(nabob_framework: &signer, accounts: vector<AccountMap>) {
    ...
    fun create_account(nabob_framework: &signer, account_address: address, balance: u64): signer {
        if (account::exists_at(account_address)) {
            ...
            nabob_coin::mint(nabob_framework, account_address, balance);
            account
        }
    }
    ...
    fun create_employee_validators(
        ...
        vector::for_each_ref(&employees, |employee_group| {
            ...
            while (j < num_employees_in_group) {
                ...
                let total = coin::balance<NabobCoin>(*account);
                let coins = coin::withdraw<NabobCoin>(&employee, total);
    ...
    fun create_initialize_validators_with_commission(
        ...
        vector::for_each_ref(&validators, |validator| {
            ...
            create_initialize_validator(nabob_framework, validator, use_staking_contract);
        });

        // Destroy the nabob framework account's ability to mint coins now that we're done with setting up the initial
        // validators.
        nabob_coin::destroy_mint_cap(nabob_framework);
    ...
    fun create_initialize_validators(nabob_framework: &signer, validators: vector<ValidatorConfiguration>) {
        ...
        create_initialize_validators_with_commission(nabob_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(nabob_framework, validator.owner_address, validator.stake_amount);
        create_account(nabob_framework, validator.operator_address, 0);
        create_account(nabob_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(nabob_framework: &signer) {
        chain_status::set_genesis_end(nabob_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        features::change_feature_flags_for_verification(nabob_framework, vector[1, 2], vector[]);
        initialize_nabob_coin(nabob_framework);
        nabob_governance::initialize_for_verification(
            nabob_framework,
            min_voting_threshold,
            required_proposer_stake,
            voting_duration_secs
        );
        create_accounts(nabob_framework, accounts);
        create_employee_validators(employee_vesting_start, employee_vesting_period_duration, employees);
        create_initialize_validators_with_commission(nabob_framework, true, validators);
        set_genesis_end(nabob_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@nabob_framework), 1);
    ...
    fun test_create_account(nabob_framework: &signer) {
        setup();
        initialize_nabob_coin(nabob_framework);

        let addr = @0x121341; // 01 -> 0a are taken
        let test_signer_before = create_account(nabob_framework, addr, 15);
        let test_signer_after = create_account(nabob_framework, addr, 500);
        assert!(test_signer_before == test_signer_after, 0);
        assert!(coin::balance<NabobCoin>(addr) == 15, 1);
    }

    #[test(nabob_framework = @0x1)]
    fun test_create_accounts(nabob_framework: &signer) {
        setup();
        initialize_nabob_coin(nabob_framework);
        ...
        create_accounts(nabob_framework, accounts);
        assert!(coin::balance<NabobCoin>(addr0) == 12345, 0);
        assert!(coin::balance<NabobCoin>(addr1) == 67890, 1);

        create_account(nabob_framework, addr0, 23456);
        assert!(coin::balance<NabobCoin>(addr0) == 12345, 2);
    }

    #[test(nabob_framework = @0x1, root = @0xabcd)]
    fun test_create_root_account(nabob_framework: &signer) {
        use nabob_framework::aggregator_factory;
