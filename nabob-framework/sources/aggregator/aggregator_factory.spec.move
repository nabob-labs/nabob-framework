spec nabob_framework::aggregator_factory {
    use nabob_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the Nabob framework is the caller and that the
    /// AggregatorFactory resource will move under the Nabob framework account.
    /// Criticality: High
    ...
    /// resource transfers to the Nabob framework account.
    ...
    /// under the Nabob account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the Nabob
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the Nabob framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the Nabob framework
    /// address.
    ...
    spec initialize_aggregator_factory(nabob_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@nabob_framework);
    }
