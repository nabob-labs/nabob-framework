spec nabob_framework::account {
    ...
    /// to the OriginatingAddress under the nabob_framework account.
    ...
    /// Only the address `@nabob_framework` can call.
    /// OriginatingAddress does not exist under `@nabob_framework` before the call.
    spec initialize(nabob_framework: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(nabob_addr);
        ensures exists<OriginatingAddress>(nabob_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @nabob_framework || new_address == @nabob_token;
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@nabob_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@nabob_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@nabob_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@nabob_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@nabob_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@nabob_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
