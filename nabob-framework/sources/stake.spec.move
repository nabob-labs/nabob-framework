spec nabob_framework::stake {
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (NabobCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@nabob_framework) ==> validator_set_is_valid();
        // After genesis, `NabobCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<NabobCoinCapabilities>(@nabob_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@nabob_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@nabob_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@nabob_framework))) ==>
            old(global<ValidatorSet>(@nabob_framework)) == global<ValidatorSet>(@nabob_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@nabob_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@nabob_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@nabob_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(nabob_framework: &signer) {
        ...
        let nabob_addr = signer::address_of(nabob_framework);
        aborts_if !system_addresses::is_nabob_framework_address(nabob_addr);
        aborts_if exists<ValidatorSet>(nabob_addr);
        aborts_if exists<ValidatorPerformance>(nabob_addr);
        ensures exists<ValidatorSet>(nabob_addr);
        ensures global<ValidatorSet>(nabob_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(nabob_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@nabob_framework);
        aborts_if !exists<ValidatorSet>(@nabob_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@nabob_framework);
        let post p_validator_set = global<ValidatorSet>(@nabob_framework);
    ...
    )
    {
        ...
        let bool_find_validator = !option::spec_is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::spec_is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::spec_is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@nabob_framework);
        ...
        aborts_if !exists<coin::CoinStore<NabobCoin>>(addr);
        include coin::DepositAbortsIf<NabobCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<NabobCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<NabobCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<NabobCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@nabob_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<NabobCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@nabob_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = nabob_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@nabob_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = nabob_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include nabob_framework::nabob_coin::ExistsNabobCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@nabob_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@nabob_framework);
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<NabobCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<NabobCoin>>(addr);
        modifies global<coin::CoinInfo<NabobCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<NabobCoin> { amount: amount };
    }
    ...
    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@nabob_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let nabob = @nabob_framework;
        let pre_validator_set = global<ValidatorSet>(nabob);
        let post validator_set = global<ValidatorSet>(nabob);
        let staking_config = global<staking_config::StakingConfig>(nabob);
    ...
    spec configure_allowed_validators(nabob_framework: &signer, accounts: vector<address>) {
        let nabob_framework_address = signer::address_of(nabob_framework);
        aborts_if !system_addresses::is_nabob_framework_address(nabob_framework_address);
        let post allowed = global<AllowedValidators>(nabob_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@nabob_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<NabobCoinCapabilities>(@nabob_framework);
        requires exists<ValidatorPerformance>(@nabob_framework);
        requires exists<ValidatorSet>(@nabob_framework);
        requires exists<StakingConfig>(@nabob_framework);
        requires exists<StakingRewardsConfig>(@nabob_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@nabob_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@nabob_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = nabob_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = nabob_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate
