spec nabob_framework::chain_status {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: The end of genesis mark should persist throughout the entire life of the chain.
    /// Criticality: Medium
    /// Implementation: The Nabob framework account should never drop the GenesisEndMarker resource.
    ...
    spec set_genesis_end(nabob_framework: &signer) {
        use std::signer;
        pragma verify = true;
        ...
        /// [high-level-req-3]
        aborts_if exists<GenesisEndMarker>(@nabob_framework);
        /// [high-level-req-1]
        ensures global<GenesisEndMarker>(@nabob_framework) == GenesisEndMarker {};
    }
