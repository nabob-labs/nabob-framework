module nabob_std::smart_vector {
    use std::error;
    use std::vector;
    use nabob_std::big_vector::{Self, BigVector};
    use nabob_std::math64::max;
    use nabob_std::type_info::size_of_val;
    ...
    public inline fun for_each<T: store>(self: SmartVector<T>, f: |T|) {
        nabob_std::smart_vector::reverse(&mut self); // We need to reverse the vector to consume it efficiently
        nabob_std::smart_vector::for_each_reverse(self, |e| f(e));
    }

    /// Apply the function to each T in the vector, consuming it.
    public inline fun for_each_reverse<T>(self: SmartVector<T>, f: |T|) {
        let len = nabob_std::smart_vector::length(&self);
        while (len > 0) {
            f(nabob_std::smart_vector::pop_back(&mut self));
            len = len - 1;
        };
    ...
    public inline fun map_ref<T1, T2: store>(
        ...
        let result = nabob_std::smart_vector::new<T2>();
        nabob_std::smart_vector::for_each_ref(self, |elem| nabob_std::smart_vector::push_back(&mut result, f(elem)));
        result
    }
    ...
    public inline fun map<T1: store, T2: store>(
        ...
        let result = nabob_std::smart_vector::new<T2>();
        nabob_std::smart_vector::for_each(self, |elem| push_back(&mut result, f(elem)));
        result
    }
    ...
    public inline fun filter<T: store + drop>(
        ...
        let result = nabob_std::smart_vector::new<T>();
        nabob_std::smart_vector::for_each(self, |elem| {
            if (p(&elem)) nabob_std::smart_vector::push_back(&mut result, elem);
        });
        result
    }
    ...
    public inline fun zip<T1: store, T2: store>(self: SmartVector<T1>, v2: SmartVector<T2>, f: |T1, T2|) {
        ...
        nabob_std::smart_vector::reverse(&mut self);
        nabob_std::smart_vector::reverse(&mut v2);
    ...
    public inline fun zip_reverse<T1, T2>(
        ...
        assert!(len == nabob_std::smart_vector::length(&v2), 0x20005);
        while (len > 0) {
            f(nabob_std::smart_vector::pop_back(&mut self), nabob_std::smart_vector::pop_back(&mut v2));
            len = len - 1;
        };
        nabob_std::smart_vector::destroy_empty(self);
    ...
    public inline fun zip_map<T1: store, T2: store, NewT: store>(
        ...
        // We can't use the constant ESMART_VECTORS_LENGTH_MISMATCH here as all calling code would then need to define it
        // due to how inline functions work.
        assert!(nabob_std::smart_vector::length(&self) == nabob_std::smart_vector::length(&v2), 0x20005);

        let result = nabob_std::smart_vector::new<NewT>();
        nabob_std::smart_vector::zip(self, v2, |e1, e2| push_back(&mut result, f(e1, e2)));
        result
    }
    ...
    public inline fun zip_map_ref<T1, T2, NewT: store>(
        ...
        let result = nabob_std::smart_vector::new<NewT>();
        nabob_std::smart_vector::zip_ref(self, v2, |e1, e2| push_back(&mut result, f(e1, e2)));
        result
    }
