...
module nabob_std::smart_table {
    use std::error;
    use std::vector;
    use nabob_std::nabob_hash::sip_hash_from_value;
    ...
    public inline fun for_each_ref<K, V>(self: &SmartTable<K, V>, f: |&K, &V|) {
        let i = 0;
        while (i < nabob_std::smart_table::num_buckets(self)) {
            vector::for_each_ref(
                nabob_std::table_with_length::borrow(nabob_std::smart_table::borrow_buckets(self), i),
                |elem| {
                    let (key, value) = nabob_std::smart_table::borrow_kv(elem);
                    f(key, value)
                }
            );
            i = i + 1;
    ...
    public inline fun for_each_mut<K, V>(self: &mut SmartTable<K, V>, f: |&K, &mut V|) {
        ...
        while (i < nabob_std::smart_table::num_buckets(self)) {
            vector::for_each_mut(
                table_with_length::borrow_mut(nabob_std::smart_table::borrow_buckets_mut(self), i),
                |elem| {
                    let (key, value) = nabob_std::smart_table::borrow_kv_mut(elem);
                    f(key, value)
                }
            );
            i = i + 1;
    ...
    public inline fun any<K, V>(
        ...
        let i = 0;
        while (i < nabob_std::smart_table::num_buckets(self)) {
            found = vector::any(table_with_length::borrow(nabob_std::smart_table::borrow_buckets(self), i), |elem| {
                let (key, value) = nabob_std::smart_table::borrow_kv(elem);
                p(key, value)
            });
            if (found) break;
