module nabob_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `nabob_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `nabob_framework::chain_id::get()` will
    /// return whichever ID was passed to `nabob_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::nabob_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(account_address(&type_info) == @nabob_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags_for_testing(&fx, vector[features::get_nabob_stdlib_chain_id_feature()], vector[]);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @nabob_std;
